...
public class BTreeImpl<T extends Comparable<T>> implements BTree<T> {
	...
	private void depthLeftOrderRec(LinkedList<BNode<T>> list, BNode<T> node) {
		list.add(node);
		for (BNode<T> e : node.getChildren()) {
			depthLeftOrderRec(list, e);
		}
	}
	...
	private void promote(BNode<T> node, BNode<T> rightChild, BNode<T> leftChild) {
	...
	private void promote(BNode<T> node) {
		this.promote(root, new BNode<T>(order), new BNode<T>(order));
	}
